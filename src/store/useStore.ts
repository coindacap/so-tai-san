...
interface Actions {
  ...
  ensureStockAsset: (symbol: string, name?: string) => string
  /**
   * Đồng bộ hold cổ phiếu từ SSI (snapshot).
   * Điều chỉnh delta qty về đúng SSI; giá vốn theo costPrice khi mua thêm.
   */
  applySsiHoldings: (input: {
    lines: { symbol: string; name?: string; qty: number; avgCost: number }[]
    accountNo?: string
    syncedAt?: string
...
export const useStore = create<Store>()(
  persist(
    (set, get) => ({
      ...
      ensureStockAsset: (symbol, name) => {
        const state = get()
        const sym = symbol.trim().toUpperCase()
        const existing = state.assets.find(
          (a) => a.symbol === sym && a.assetClass === 'stock',
        )
        if (existing) return existing.id
        const t = nowIso()
        const id = uid()
      ...
      applySsiHoldings: (input) => {
        ...
        if (!input.lines?.length) {
          return { ok: false, error: 'Không có vị thế cổ phiếu' }
        }
        if (hasAnyData(state)) {
          pushSafetyBackup(
            'import',
        ...
        const t = nowIso()
        const tradedAt = input.syncedAt || t
        const noteBase = `SSI sync${input.accountNo ? ` · TK ${input.accountNo}` : ''}`
        const newTxs: Transaction[] = []
        const quotePatch: Record<string, PriceQuote> = {}
        let touched = 0
        ...
        const targets = new Map<
          string,
          { assetId: string; qty: number; avgCost: number }
        >()
        for (const line of input.lines) {
          if (line.qty <= 0) continue
          const assetId = get().ensureStockAsset(line.symbol, line.name)
          stockIds.add(assetId)
          targets.set(assetId, {
            assetId,
            qty: line.qty,
            avgCost: Math.max(0, line.avgCost),
          })
        }

        // Zero stocks not in SSI list (còn hold)
        for (const id of stockIds) {
          if (targets.has(id)) continue
          const hold = qtyHoldAt(get(), id)
          if (hold <= 0) continue
          const pos = get().assets.find((a) => a.id === id)
          newTxs.push({
            id: uid(),
            kind: 'adjust',
            assetId: id,
            ...
            tradedAt,
            venue: 'SSI',
            note: `${noteBase} · xóa hold ${pos?.symbol || id} (không còn trên SSI)`,
            createdAt: t,
            updatedAt: t,
          })
          touched++
        ...
        for (const [, tg] of targets) {
          const hold = qtyHoldAt(get(), tg.assetId)
          // Apply pending outs already in newTxs for other assets only —
          // for this asset, hold is still current store (we batch set at end)
          const delta = tg.qty - hold
          const px = tg.avgCost > 0 ? tg.avgCost : 0
          if (Math.abs(delta) > 1e-9) {
            newTxs.push({
              ...
              priceCurrency: 'VND',
              fee: 0,
              counterAssetId: tg.assetId,
              counterQty: 0,
              counterCostVnd: Math.abs(delta) * px,
              tradedAt,
              venue: 'SSI',
              note: `${noteBase} · đồng bộ hold`,
              createdAt: t,
              updatedAt: t,
            })
            touched++
          }
          // Quote = giá vốn (có thể cập nhật giá TT sau)
          if (px > 0) {
            quotePatch[tg.assetId] = {
              assetId: tg.assetId,
              price: px,
              currency: 'VND',
              label: 'SSI giá vốn',
        ...
        // Fix: delta used store hold before batch — for multiple lines OK per asset once.
        // But zero-outs don't affect other targets' hold. Good.

        set((s) => ({
          transactions: [...s.transactions, ...newTxs],
          quotes: { ...s.quotes, ...quotePatch },
        }))

        return {
          ok: true,
          message: `Đã đồng bộ ${targets.size} mã CK từ SSI (${touched} điều chỉnh)`,
        }
      },
